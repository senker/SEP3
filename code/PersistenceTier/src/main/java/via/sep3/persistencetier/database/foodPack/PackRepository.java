...
public interface PackRepository extends JpaRepository<FoodPack, Integer> {
    ...
    @Query(value="select * from FoodPack p where p.title like CONCAT(CONCAT('%', ?1), '%') and p.is_prepared=?2 and p.type=?3 and p.price<=?4 and p.seller.address.postcode=?5", nativeQuery = true)
    Stream<FoodPack> searchPacks(String title, boolean isPrepared, String type, Double price, Integer postCode);


    @Query(value="select * from FoodPack p where p.title like CONCAT(CONCAT('%', ?1), '%') and p.is_prepared=?2 and p.type=?3 and p.seller.address.postcode=?4", nativeQuery = true)
